...
```
M_CREDIT_CARD
  ├── M_IMPORT_BATCH (cardId)
  │     └── M_TRANSACTION (importBatchId, cardId)
  ├── M_TRANSACTION (cardId)
...
| strStatus | VARCHAR(20) | "ACTIVE" or "DELETED" |
| dateCreation | DATETIME | Auto-set |
| dateModification | DATETIME | Auto-set |
...
CREATE TABLE M_IMPORT_BATCH (
  ...
  CONSTRAINT FK_IMPORT_BATCH_CARD FOREIGN KEY (cardId) REFERENCES M_CREDIT_CARD(id)
)
GO
CREATE INDEX IDX_IMPORT_BATCH_CARD ON M_IMPORT_BATCH (cardId)
GO
...
-- ============================================================
-- M_TRANSACTION
-- (depends on M_CREDIT_CARD, M_IMPORT_BATCH, M_SUBCATEGORY)
-- ============================================================
...
CREATE TABLE M_TRANSACTION (
  ...
  strStatus         VARCHAR(20)       NOT NULL DEFAULT 'ACTIVE',  -- ACTIVE | DELETED
  dateCreation      DATETIME          NULL DEFAULT [dbo].[fn_getcurrentdate](),
  dateModification  DATETIME          NULL DEFAULT [dbo].[fn_getcurrentdate](),
  CONSTRAINT PK_TRANSACTION PRIMARY KEY (id),
  CONSTRAINT FK_TRANSACTION_CARD FOREIGN KEY (cardId) REFERENCES M_CREDIT_CARD(id),
...
GO
CREATE INDEX IDX_TRANSACTION_CARD ON M_TRANSACTION (cardId)
GO
CREATE INDEX IDX_TRANSACTION_DATE ON M_TRANSACTION (dateTransaction)
GO
CREATE INDEX IDX_TRANSACTION_CARD_DATE ON M_TRANSACTION (cardId, dateTransaction)
GO
...
BEGIN
  ...
  INSERT INTO M_TRANSACTION (
    cardId, importBatchId, dateTransaction, strDescription,
...
BEGIN
  SET NOCOUNT ON;
  DECLARE @tableInsertedId TABLE (id UNIQUEIDENTIFIER);
  INSERT INTO M_IMPORT_BATCH (cardId, decUsdExchangeRate, decBalanceAtImport)
  OUTPUT INSERTED.id INTO @tableInsertedId
  VALUES (@cardId, @decUsdExchangeRate, @decBalanceAtImport);
  SET @insertedId = (SELECT id FROM @tableInsertedId);
END
GO
...
Step 8 — Persist results
  a. If interest != 0:
     Create M_TRANSACTION: cardId, dateTransaction=closingDate, strDescription="INTEREST CHARGE",
       strCurrency="PEN", decAmount=interest, decAmountPen=interest, strStatus='ACTIVE'
...
| Function Name | Method | Route | Request | Response |
|---|---|---|---|---|
| `transaction_GetByCard` | GET | `/transaction/list` | Query: `cardId`, `dateFrom?`, `dateTo?` | `Transaction[]` |
...
| Function Name | Method | Route | Request | Response |
|---|---|---|---|---|
| `importBatch_Upload` | POST | `/import-batch/upload` | `multipart/form-data`: `file` (.xlsx), `cardId`, `decBalanceAtImport`, `decUsdExchangeRate` | `ImportReviewPayload` |
| `importBatch_Confirm` | POST | `/import-batch/{importBatchId}/confirm` | `ImportConfirmRequest` | `{ savedCount: number }` |
| `importBatch_GetByCard` | GET | `/import-batch/list` | Query: `cardId` | `ImportBatch[]` |
