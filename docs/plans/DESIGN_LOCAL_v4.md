...
## Changelog from v1.2 → v1.3

- Added **Recurrent Dashboard** UI flow specification: display of all recurrent transactions with DONE/PENDING/NOT_YET_DUE status, "Mark as done" and "Link transaction" actions.
- Added **Transaction Explorer** feature: new top-level route (`/explorer`) for filtering transactions by category/subcategory.
- Added **Monthly Income/Expense Dashboard** (`/dashboard/monthly`): month-by-month income vs. expense summary with subcategory breakdown.
- Added **Cycle Expense Dashboard** (`/dashboard/cycle`): per-credit-card expense summary for the current billing cycle, reusing `ConciliationCalculatorService.buildCycleWindow()`.
- Added §7.6 `DashboardCalculatorService` (pure, stateless) with `computeMonthly()` and `computeCycleSummary()`.
- Updated component tree: added `explorer/` and `dashboard/` lazy-loaded features.
- Updated route catalog: added `EXPLORER`, `DASHBOARD_MONTHLY`, `DASHBOARD_CYCLE`.

---

## Changelog from v1.1 → v1.2

- `ExcelParserService`: Excel column names extracted to named constants at the top of the service for easy maintenance.
- `RawImportRow`: Moved from `excel-parser.service.ts` to `src/app/core/models/import-batch.model.ts` alongside other domain interfaces. All field names converted from Spanish (`fecha`, `descripcion`, `moneda`, `monto`) to English (`dateTransaction`, `description`, `currency`, `amount`). Added `amountPen` field (PEN-converted amount, computed at parse time). `parseFile()` now accepts `usdExchangeRate` to compute `amountPen` inline.
- All downstream references to `RawImportRow` field names updated (DuplicationLogicService, Import Wizard data flow).
- Import Wizard: promoted from 2-step to 3-step flow. Step 2 (Review) now ends with "Next", deferring all writes to Step 3 (Finalize). Step 3 saves transactions, runs automatic recurrent matching against the current cycle, checks storage, and redirects to the conciliation page.
...
7. [Business Logic Services](#7-business-logic-services)
   ...
   - [7.6 DashboardCalculatorService](#76-dashboardcalculatorservice)
...
- Recurrent payment reminder dashboard with DONE/PENDING/NOT_YET_DUE status
- Transaction explorer: filter all transactions by category/subcategory
...
/** One row as parsed directly from the Excel file, before any business logic. */
export interface RawImportRow {
  dateTransaction: string;          // YYYY-MM-DD derived from the Fecha column
  description: string;              // trimmed description from Excel
  currency: TransactionCurrency;    // 'S/' → PEN, '$' → USD
  amount: number;                   // raw amount from Excel (sign preserved as-is from file)
  amountPen: number;                // always in PEN; USD rows = amount × usdExchangeRate at parse time
  strOperationNumber?: string;      // debit accounts only; used for D1 duplication rule
}
```
...
Parses an `.xlsx` File object into `RawImportRow[]` using SheetJS. Column header names are declared as constants at the top for easy maintenance when the bank changes its export format. For debit account imports, the parser reads `N° Operacion` (falling back to `Operacion`) and maps it to `strOperationNumber`. `amountPen` is computed at parse time using the supplied exchange rate so downstream services receive a fully resolved row.
...
// ----- Column header constants — match exact Excel column names from bank exports -----
...
const COL_OPERACION_ALT = 'Operacion';   // fallback for banks that omit the degree symbol
// --------------------------------------------------------------------------------------

@Injectable({ providedIn: 'root' })
export class ExcelParserService {

  async parseFile(file: File, usdExchangeRate: number): Promise<RawImportRow[]> {
    ...
    return rows
      ...
      .map(r => {
        ...
        const amount   = Number(r[COL_MONTO]);
        return {
...
export interface AnnotatedImportRow {
  ...
  subcategoryId?: string;            // pre-populated by CategoryMatchingService
  note?: string;                     // free-text note added via context menu
  pendingFlag?: boolean;             // true if user marked "Mark as Pending"
  ...
  private annotateRow(
    ...
    for (const col of collections) {
      const incomingHit = col.strings.find(s =>
        row.description.toLowerCase().includes(s.toLowerCase())
      );
      if (incomingHit) {
...
export class CategoryMatchingService {
   ...
   *  or undefined if no rule matched. */
...
Pure service for computing monthly income/expense summaries and per-card cycle expense summaries. Depends only on `ConciliationCalculatorService` for cycle window computation.
...
export interface SubcategoryTotal {
  ...
  categoryName: string;
  total: number;          // sum of decAmountPen; negative = net expense
}

export interface MonthlyDashboardData {
  month: string;                     // 'YYYY-MM'
  totalIncome: number;               // sum where decAmountPen > 0
  totalExpenses: number;             // sum where decAmountPen < 0 (negative value; display as abs)
  netAmount: number;                 // totalIncome + totalExpenses
  bySubcategory: SubcategoryTotal[];
  uncategorizedTotal: number;        // sum for transactions with no subcategoryId
}

export interface CycleDashboardCardData {
  card: CreditCard;
  cycleWindow: CycleWindow;
  totalExpenses: number;             // abs(sum decAmountPen < 0 in window)
  totalPayments: number;             // sum decAmountPen > 0 in window
  transactionCount: number;
  topSubcategories: SubcategoryTotal[];  // top 3 by abs(total)
}
...
export class DashboardCalculatorService {
  ...
  /** Compute income/expense totals and subcategory breakdown for a given month.
   *  Includes ACTIVE and PENDING transactions; excludes DELETED. */
  computeMonthly(
    month: string,   // 'YYYY-MM'
    transactions: Transaction[],
    ...
    const totalIncome   = inMonth.filter(t => t.decAmountPen > 0)
                                 .reduce((s, t) => s + t.decAmountPen, 0);
    const totalExpenses = inMonth.filter(t => t.decAmountPen < 0)
                                 .reduce((s, t) => s + t.decAmountPen, 0);
    ...
    return {
      ...
      uncategorizedTotal: uncategorized.reduce((s, t) => s + t.decAmountPen, 0),
    };
  }

  /** Compute cycle expense summary for a single credit card. */
  computeCycleSummary(
    card: CreditCard,
    todayDate: string,
    transactions: Transaction[],
    ...
    const totalExpenses = Math.abs(
      cycleTransactions.filter(t => t.decAmountPen < 0)
                       .reduce((s, t) => s + t.decAmountPen, 0)
    );
    const totalPayments = cycleTransactions.filter(t => t.decAmountPen > 0)
                                           .reduce((s, t) => s + t.decAmountPen, 0);

    const bySub = this.groupBySubcategory(
      cycleTransactions.filter(t => t.subcategoryId != null),
      subcategories,
      categories
    );
  ...
  private groupBySubcategory(
    ...
    return Array.from(map.entries()).map(([id, total]) => {
      ...
      return {
        subcategoryId: id,
        subcategoryName: sub?.strName ?? '(unknown)',
        categoryName: cat?.strName ?? '(unknown)',
        total,
      };
    });
  }
...
│   │   │   ├── import-batch.model.ts      ← includes RawImportRow
│   │   │   ├── category.model.ts
...
│   │   │   └── dashboard-calculator.service.ts   ← section 7.6
│   │   │
│   │   └── components/
...
│       │   └── pages/
│       │       ├── import-wizard/
│       │       │   ├── import-wizard.component.ts   ← step container (3 steps)
...
│       │       └── step-finalize/                   ← Step 3
│       │           ├── step-finalize.component.ts
│       │           └── step-finalize.component.html
│       │
...
│       │
│       ├── explorer/                      ← lazy-loaded (NEW)
│       │   ├── explorer.routes.ts
│       │   └── pages/
│       │       └── transaction-explorer/
│       │           ├── transaction-explorer.component.ts
│       │           └── transaction-explorer.component.html
│       │
│       └── dashboard/                     ← lazy-loaded (NEW)
│           ├── dashboard.routes.ts
│           ├── services/
│           │   └── (DashboardCalculatorService lives in core/services — no feature service needed)
│           └── pages/
│               ├── monthly-dashboard/
│               │   ├── monthly-dashboard.component.ts
│               │   └── monthly-dashboard.component.html
│               └── cycle-dashboard/
│                   ├── cycle-dashboard.component.ts
│                   └── cycle-dashboard.component.html
```
...
export const APP_ROUTES: Routes = [
  {
    ...
    children: [
      ...
      },
      {
        ...
        loadChildren: () =>
          import('../features/recurrent-transactions/recurrent-transactions.routes').then(m => m.RECURRENT_ROUTES),
      },
      {
        path: 'explorer',
        loadChildren: () =>
...
The import wizard now consists of **three steps**: **Upload**, **Review**, and **Finalize**. An intermediate state object lives in `ImportWizardState` (a `BehaviorSubject` in `import-wizard.state.ts`) and is shared between all three step components via a service.

No data is written to localStorage until the user confirms in **Step 3**. Steps 1 and 2 are purely in-memory.
...
export interface ImportWizardState {
  ...
  contextTransactions: string[];   // ids of existing transactions within 5 days before earliest incoming date
}
...
1. User selects an account from a `<mat-select>` bound to both `CreditCardService.cards$` and `DebitAccountService.accounts$`.
2. User picks an `.xlsx` file via `<input type="file">`.
3. User enters current balance (positive number) and USD/PEN exchange rate.
4. On "Next":
   - `ExcelParserService.parseFile(file, usdExchangeRate)` → `RawImportRow[]` (each row includes `amountPen`, computed inline by the parser).
   - `DuplicationLogicService.annotate(rawRows, existingTransactions, collections)` → `AnnotatedImportRow[]`.
   - `CategoryMatchingService.match(description, rules)` for each row → pre-populate `subcategoryId`.
   - Compute `contextTransactions`: earliest `dateTransaction` in incoming rows − 5 days window.
   - `ImportWizardStateService.patch(...)` → navigate to Step 2.

**Step 2 — Review (`StepReviewComponent`)**

1. Reads state from `ImportWizardStateService.state$`.
2. Rows grouped by `dateTransaction` descending.
3. AUTO_DUPLICATE rows: unchecked and disabled. POTENTIAL_DUPLICATE rows: highlighted with matching sub-row.
4. Context menu per row: "Add additional info" (note dialog), "Mark as Pending" (sets `pendingFlag = true`).
5. Context transactions rendered below with trash icon → `TransactionService.softDelete(id)` (only localStorage write in Step 2).
6. On "Next →": persist row changes to state, navigate to Step 3. **No ImportBatch or Transaction records created.**

**Step 3 — Finalize (`StepFinalizeComponent`)**

Read-only summary of what will be saved, then on "Save & Finish":

**a. Create the ImportBatch:**
```typescript
const batch = importBatchService.save({
  accountId, accountType, dateImport: today,
  decUsdExchangeRate: usdExchangeRate,
  decBalanceAtImport: accountType === 'CREDIT_CARD' ? -Math.abs(currentBalance) : currentBalance,
});
```
...
const transactions = checkedRows.map(row => ({
...
const savedTransactions = transactionService.saveMany(transactions);
```

**c. Update the account's current balance:**
```typescript
if (accountType === 'CREDIT_CARD') {
  creditCardService.save({ ...card, decCurrentBalance: -Math.abs(currentBalance) });
} else {
  debitAccountService.updateBalance(accountId, currentBalance);
}
```

**d. Run recurrent transaction matching for the current cycle:**
```typescript
const todayDate = new Date().toISOString().slice(0, 10);
const allActive = transactionService.getActive();
const newMatches = recurrentMatchingService.findAutomaticMatches(
  recurrentTransactionService.getAll(),
  recurrentTransactionService.getAllMatches(),
  allActive, todayDate
);
for (const result of newMatches) {
  recurrentTransactionService.saveMatch({
    recurrentTransactionId: result.recurrentTransaction.id,
    strIterationKey: result.iterationKey,
    transactionId: result.matchedTransaction.id,
    boolDone: true, dateDone: todayDate, strMatchMode: 'AUTOMATIC',
  });
}
```

**e. Check storage and finalize:**
```typescript
importWizardStateService.reset();
if (storageService.getTotalBytes() > WARN_THRESHOLD_BYTES) storageWarningService.show();
router.navigate([ROUTES.CONCILIATION]);
```

**What is written to localStorage on Step 3 confirm:**
...
| `pfmg_recurrent_matches` | 0–N new `RecurrentTransactionMatch` records for auto-matched recurrents |
...
1. If `interestAmount !== 0`: save interest `Transaction` with `dateTransaction = closingDate`, `strDescription = 'INTERES'`.
2. `ConciliationService.saveCycleClose(...)`.
3. `ConciliationService.saveCycleSnapshot(...)`.
4. Check storage size; show warning if > 4 MB.
...
**Route:** `RECURRENT_DASHBOARD` → `recurrent-transactions/dashboard`  
**Component:** `RecurrentDashboardComponent`

### Data loaded on init

```typescript
recurrents     = recurrentTransactionService.getAll();
matches        = recurrentTransactionService.getAllMatches();
allActive      = transactionService.getActive();
cards          = creditCardService.getAll();
debitAccounts  = debitAccountService.getAll();
todayDate      = new Date().toISOString().slice(0, 10);
```

### Row computation

For each `RecurrentTransaction`, compute:

```typescript
iterationKey = recurrentMatchingService.getCurrentIterationKey(r.strFrequency, todayDate);
match        = recurrentTransactionService.getMatchForIteration(r.id, iterationKey);

// Status per BUSINESS_LOGIC.md §5.6
if (match?.boolDone):
  status = 'DONE'
else:
  targetDate = buildTargetDate(r, iterationKey)
  if (targetDate && today < targetDate - r.intDayRange):
    status = 'NOT_YET_DUE'
  else:
    status = 'PENDING'
```
...
|---|---|
| Name | `r.strName` |
| Account | credit card or debit account name (resolved by `accountId`) |
| Frequency | `MONTHLY` / `YEARLY` |
| Approx amount | `r.decApproxAmount` (PEN; display with sign) |
| Expected date | `targetDate` or "—" if undefined |
| Status badge | `DONE` (green) / `PENDING` (amber) / `NOT_YET_DUE` (gray) |
| Matched on | DONE only: `match.dateDone` and linked transaction amount |

Sort order: PENDING → NOT_YET_DUE → DONE.

### Actions (PENDING rows only)

**"Mark as done" button:**
```typescript
recurrentTransactionService.saveMatch({
  recurrentTransactionId: r.id,
  strIterationKey:        iterationKey,
  transactionId:          undefined,   // no linked transaction
  boolDone:               true,
  dateDone:               todayDate,
  strMatchMode:           'MANUAL',
});
...
Opens `MatDialog` listing ACTIVE transactions for the same `accountId`, sorted by `dateTransaction` DESC. User selects one:
```typescript
recurrentTransactionService.saveMatch({
  recurrentTransactionId: r.id,
  strIterationKey:        iterationKey,
...
| Key | Action |
|---|---|
| `pfmg_recurrent_matches` | One new `RecurrentTransactionMatch` per action |
...
**Component:** `TransactionExplorerComponent`

### Purpose

Allows the user to find all transactions belonging to a category or subcategory. Answers questions like "when was the last time I had a haircut?" or "how much have I spent on subscriptions this year?".

### Filter panel

```
[ Category ▼ ]   [ Subcategory ▼ ]   [ Account (optional) ▼ ]
```

- **Category `<mat-select>`**: populated from `CategoryService.getCategories()`.
- **Subcategory `<mat-select>`**: populated reactively from `CategoryService.getSubcategoriesByCategoryId(selectedCategoryId)` when a category is chosen.
- **Account filter (optional)**: `<mat-select>` listing all credit cards + debit accounts with an "All accounts" default.

No search button needed — results update reactively as the user changes the filters.

### Results list

Filtered set: all transactions where:
- `strStatus !== 'DELETED'`
- `subcategoryId === selectedSubcategoryId`
- `accountId === selectedAccountId` (if account filter is set)

Sorted by `dateTransaction` DESC (most recent first — supports the "when was the last time" use case).
...
| Amount | `t.decAmountPen` (PEN; colored red for negative, green for positive) |
| Status badge | ACTIVE / PENDING |

### Empty states

- No category selected: prompt "Select a category to start exploring".
- No subcategory selected (category chosen): prompt "Select a subcategory to see transactions".
- No results: "No transactions found for this subcategory."

### Data sources (no new service needed — inline filtering in component)
...
No localStorage writes occur in this feature.

---

## 13. Monthly Dashboard — Data Flow

**Route:** `DASHBOARD_MONTHLY` → `dashboard/monthly`  
**Component:** `MonthlyDashboardComponent`

### Month navigation

- Default: current month (`new Date().toISOString().slice(0, 7)` → `'YYYY-MM'`).
- "← Previous" / "Next →" buttons adjust the month string by one month.
- Header shows the full month name, e.g., "May 2026".

### Data computation

On every month change, call:

```typescript
const data: MonthlyDashboardData = dashboardCalculatorService.computeMonthly(
...
| Income (PEN) | Expenses (PEN) | Net (PEN) |
|---|---|---|
| `data.totalIncome` (green) | `abs(data.totalExpenses)` (red) | `data.netAmount` (colored by sign) |

**Subcategory breakdown table** (sorted by `abs(total)` descending):

| Category | Subcategory | Total (PEN) |
|---|---|---|
| Alimentación | Supermercado | −350.00 |
| Entretenimiento | Streaming | −89.90 |
| … | … | … |
| *(Uncategorized)* | — | `data.uncategorizedTotal` |

Expense rows are shown in red, income rows in green.

### Data sources

```typescript
transactions  = transactionService.getAll();          // subscribed via transactions$
subcategories = categoryService.getSubcategories();
categories    = categoryService.getCategories();
```
...
**Route:** `DASHBOARD_CYCLE` → `dashboard/cycle`  
**Component:** `CycleDashboardComponent`

### Purpose

Shows each credit card's current billing cycle summary: total expenses, payments/credits, and top spending subcategories. Debit accounts are excluded (no billing cycle concept).
...
const cardSummaries: CycleDashboardCardData[] = cards.map(card =>
  dashboardCalculatorService.computeCycleSummary(card, todayDate, txns, subs, cats)
);
```

Each `computeCycleSummary` call uses `ConciliationCalculatorService.buildCycleWindow()` to derive the current cycle window for that card's `intClosingDay`.

### Display

One summary widget per credit card:

```
┌─────────────────────────────────────┐
│  BCP Visa Gold                      │
│  Cycle: 2026-04-16 → 2026-05-15     │
│                                     │
│  Expenses   −1,250.50 PEN           │
│  Payments      500.00 PEN           │
│  Current balance: −750.50 PEN       │
│                                     │
│  Top spending:                      │
│    Supermercado      −350.00        │
│    Restaurantes      −210.00        │
...
- `Expenses`: `data.totalExpenses` (absolute value, shown in red)
- `Payments`: `data.totalPayments` (shown in green)
- `Current balance`: `card.decCurrentBalance` (from CreditCard record; updated after each import)
- Top subcategories: up to 3 rows from `data.topSubcategories`
- If no transactions in the current cycle: "No transactions in current cycle" placeholder

### Data sources

```typescript
cards         = creditCardService.getAll();           // subscribed via cards$
transactions  = transactionService.getAll();
subcategories = categoryService.getSubcategories();
categories    = categoryService.getCategories();
```
...
`StorageService.getTotalBytes()` returns an approximate byte count. Call it after every import (Step 3) or conciliation confirm:
...
`StorageWarningBannerComponent` subscribes to the warning signal and renders a `mat-snack-bar` or a dismissible banner at the top of the page.
...
1. Add `provideHttpClient()` to the app configuration.
...
5. Migrate `RecurrentTransactionService` and `DebitAccountService` to HTTP calls.
...
The entire migration can be done incrementally: one feature service at a time.
