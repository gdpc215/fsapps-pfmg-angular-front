...
5. On "Next":
   - `ExcelParserService.parseFile(file, usdExchangeRate)` is called → returns `RawImportRow[]` (each row now includes `amountPen`, computed inline by the parser).
   - Call `DuplicationLogicService.annotate(rawRows, existingTransactions, collections)` → `AnnotatedImportRow[]`.
   - Run `CategoryMatchingService.match(description, rules)` for each row to pre-populate `subcategoryId` on each `AnnotatedImportRow`.
   - Compute `contextTransactions`: find the earliest `dateTransaction` in incoming rows, then collect IDs of all active transactions for the selected account from 5 days before that date.
...
3. Each row shows:
   ...
   - A context menu ("...") with options:
     - "Add additional info" → opens `MatDialog` with a free-text textarea; the note is stored back on the `annotatedRow.note` field.
     - "Mark as Pending" → sets `annotatedRow.pendingFlag = true`; those rows will be saved with `strStatus: 'PENDING'` in Step 3.
4. Existing transactions from the context date range are rendered below the incoming rows as read-only rows with a trash icon. Clicking trash calls `TransactionService.softDelete(id)` immediately (this is the only write that can happen in Step 2).
5. On "Next →":
   - Persist any subcategory / note / pendingFlag changes back to `ImportWizardStateService.patch({ annotatedRows: updatedRows })`.
   - Navigate to Step 3. **No ImportBatch or Transaction records are created yet.**

**Step 3 — Finalize (`StepFinalizeComponent`)**

This is the confirmation and save step. The component reads the final wizard state and presents a summary before committing.

1. Component reads state from `ImportWizardStateService.state$`.
2. Displays a read-only summary:
   - Account name
   - Number of transactions to import (checked rows count)
   - Number of auto-duplicates skipped
   - USD/PEN exchange rate applied
   - Pending transactions count (if any)
3. On "Save & Finish" button:
   ...
   **d. Run recurrent transaction matching for the current cycle:**
   ...
   const allActive = transactionService.getActive(); // includes the just-saved rows
